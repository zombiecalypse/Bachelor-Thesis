...
in the worst case, this approach will take $\TIME{\mathcal{O}(2^{\abs{Q}})}$ 
per step.
...
\begin{theorem}[Guess and check]
	...
	$w\in \Gamma^*$ for each input $x$, so that there is a deterministic Turing 
...
\begin{proof}
	...
	If on the other hand we can produce a certificate for any input and check in 
	polynomial time, then that certificate can only be $p(\abs{x})$ long for some 
...
must accellerate the computation significantly.
